...
component accessors='true' {
	...
	function buildBookHTML( required book ) {
		...
		if( book.getRenderOpts().coverPageImageFile.len() ) {
			pages.append(
				renderPartial(
					'cover-page-image',
					{ data : { coverPageImageFile : book.getRenderOpts().coverPageImageFile } },
					'',
					book
				)
			);
		...
		var renderChildren = function( tree ) {
			tree.each( ( child ) => {
				...
				if( child.type == 'page' ) {
					currentCount++;
					pages.append( renderPage( child, book ) );
				...
				'body-wrapper-top',
				{
					'data' : {
						styles : [
							fileRead(
								expandPath(
									'/commandbox-gitbook/includes/pygments/#book.getRenderOpts().codeHighlighlightTheme#.css'
								)
							),
	...
	function renderBookHTML( book ) {
		...
		// Write HTML output
		fileWrite(
			book.getHTMLExportFilePath(),
			book.getBodyWrapperTop() & book.getPages().toList( ' ' ) & book.getBodyWrapperBottom()
		);
	...
	function renderNode( required struct node, book, boolean raw = false ) {
		...
		if( node.kind == 'document' ) {
			...
			return renderPartial( node.type, node, innerContent, book ) & (
