...
component {
	...
	function run(
		...
		if(
			targetFile.len() && targetFile.listLen( '.' ) > 1 && 'pdf,html,mobi,epub'.listFindNoCase(
				targetFile.listLast( '.' )
			)
		) {
		...
		if( !isNull( arguments.pageheight ) && isNull( arguments.pagewidth ) ) {
			error(
				'You cannot set a pageheight but not a pagewidth. Please provide both, or use a pre-defined pageType.'
			);
		}
		...
		if( !isNull( arguments.pagewidth ) && isNull( arguments.pageheight ) ) {
			error(
				'You cannot set a pagewidth but not a pageheight. Please provide both, or use a pre-defined pageType.'
			);
		}
